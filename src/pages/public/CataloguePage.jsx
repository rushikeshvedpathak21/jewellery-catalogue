...
export default function CataloguePage() {
  ...
  const filtered = useMemo(() => {
    let list = products.filter((p) => p.is_visible)
    if (query) list = list.filter((p) => [p.name_en, p.name_hi, p.name_mr].some((x) => String(x || '').toLowerCase().includes(query.toLowerCase())))
    if (category) list = list.filter((p) => p.category_id === category)
    if (metal) list = list.filter((p) => p.metal_type === metal)
    if (SORTER[sortBy]) list = [...list].sort(SORTER[sortBy])
    return list
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-6">
      <div className="mb-6">
        <h1 className="font-display text-5xl font-semibold" style={{ color: 'var(--clr-section-title)', fontFamily: 'var(--font-section-title)' }}>{t('all_products')}</h1>
        <p className="mt-2 text-sm" style={{ color: 'var(--clr-section-subtitle)', fontFamily: 'var(--font-body)' }}>Browse the full collection and send inquiries directly on WhatsApp.</p>
      </div>

      <ProductFilters
        query={query}
        onQueryChange={setQuery}
        categories={visibleCategories.map((c) => ({ id: c.id, name: text(c, 'name') }))}
        ...
        {filtered.length ? (
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {filtered.map((product) => (
              <ProductCard key={product.id} product={product} categoryName={text(categories.find((c) => c.id === product.category_id) || {}, 'name')} onBlocked={setBlocked} />
            ))}
          </div>
        ) : <EmptyState title={t('no_products')} description="Try adjusting filters or search terms." />}
      </div>
